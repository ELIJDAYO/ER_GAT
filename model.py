...
#         self.smax_fc = nn.Linear(hidden_size, num_classes)
    ...
    def forward(self, x, edge_index, edge_type, seq_lengths, umask, nodal_attn, avec):
        if isinstance(x, list):
            x = torch.cat(x, dim=0)
        
        # Correctly batch edge_index and edge_type
        edge_index_cat = []
        edge_type_cat = []
        batch_offset = 0
...
class GraphNetwork_GAT(nn.Module):
    def __init__(self, num_features, num_classes, num_relations, max_seq_len, hidden_size=64, num_heads=8, dropout=0.5, no_cuda=False):
    ...
    def forward(self, x, edge_index, edge_type=None, seq_lengths=None, umask=None, nodal_attn=None, avec=None):
        if isinstance(x, list):
            x = torch.cat(x, dim=0)
...
class GraphNetwork_GAT_EdgeFeat(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr=None, seq_lengths=None, umask=None, nodal_attn=None, avec=None):
        if isinstance(x, list):
            x = torch.cat(x, dim=0)
...
class GraphNetwork_GATv2(nn.Module):
    def __init__(self, num_features, num_classes, hidden_size=64, num_heads=8, dropout=0.5, no_cuda=False):
...
class GraphNetwork_RGAT(nn.Module):
    def __init__(self, num_features, num_classes, num_relations, hidden_size=64, num_heads=8, dropout=0.5, edge_dim=1, no_cuda=False):
    ...
    def forward(self, x, edge_index, edge_type=None, edge_attr=None):
        ...
        if edge_type is not None:
            self_loop_edge_type = (edge_type.max().item() + 1) % self.num_relations  # Ensure the new edge type is within the valid range
...
#         print(f"edge_index shape: {edge_index.shape}")
#         print(f"edge_attr shape: {edge_attr.shape if edge_attr is not None else 'None'}")
#         print(f"edge_type shape: {edge_type.shape if edge_type is not None else 'None'}")
